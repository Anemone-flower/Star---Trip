...
var audio = new Audio('Sounds/fire-jump.mp3');
...
document.addEventListener('keydown', function (evt) {
    if (evt.code === 'KeyR' && !isPopupOpen) {
        const userInput = prompt('Type "Day" or "Main" to change the page:');
        if (userInput && userInput.toLowerCase() === 'day') {
...
class Dino {
          ...
          if (storedItem) {
            ...
            switch (itemRarity) {
                case "PRESTIGE":
                    ...
                    if(Reburn == 1){img.src = "dinos/Rfire_down.png"};
                    break;
                case "Legendary":
        ...
        if (storedItem) {
          ...
          switch (itemRarity) {
              case "PRESTIGE":
                  ...
                  if(Reburn == 1){img.src = "dinos/Rfire-up.png"};
                  break;
              case "Legendary":
                  img.src = "dinos/stardino-up.png";
                  break;
              case "Epic":
  ...
  function SpawnObstacle() {
    let size = RandomIntRange(60, 100);
    let type = RandomIntRange(0, 3); // 유성 생성 확률을 높입니다.
    ...
    if (type == 0) {
      ...
      // 새 장애물 생성 (이거 안되는데 ㅋㅋㅋㅋㅋㅋㅋㅋㅋ)
      obstacles.push(obstacle); // 오류 수정(?)
    } else {
      SpawnFallingStar();
    }
}
